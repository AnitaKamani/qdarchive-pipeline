...
Phase 2 prepares model-ready project metadata, combines it with the 87-division ISIC Rev. 5 taxonomy, and sends it to the ISIC Classification Engine, which selects between OpenAI models such as GPT-4o-mini and GPT-4.1-mini. Structured responses are validated against both the JSON schema and the ISIC taxonomy. Retry handling addresses transient API failures, while cross-model resume filtering prevents already completed projects from being reclassified.
